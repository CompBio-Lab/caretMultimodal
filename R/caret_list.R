...
#' @param data_list A named list of matrix-like objects, where each element is a dataset to train a separate model on.
#'   Names are preserved in the returned `caret_list` and should be unique.
#' @param method The method to train the models with. Can be a custom method or one found in `caret::modelLookup()`.
#' @param identifier_column_name A string giving the name of a column that links rows across datasets (e.g. a participant ID).
#'   If provided, this column must be present in all datasets in `data_list` for proper row matching.
#'   Use this when datasets have different numbers of rows.
#'   If `NULL`, datasets are assumed to have identical rows in the same order. Default is `NULL`.
#'
#'   \strong{Note:} Providing `identifier_column_name` disables support for custom `trControl`. A new
...
#' @param trim Logical, whether the trained models should be trimmed to save memory. Default is `TRUE`.
#' @param do_parallel Logical, whether to parallelize model training across datasets. Default is `TRUE`.
#' @param ... Additional arguments to pass to the `caret::train` function (e.g. `tuneGrid = tuneGrid`).
#' @return A `caret_list` object (a named list of trained `caret::train` models corresponding to `data_list`).
#' @examples
#' set.seed(42)
#'
